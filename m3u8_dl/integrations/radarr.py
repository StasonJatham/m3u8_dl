"""Radarr integration for automatic movie import and indexing."""
...
class RadarrUploader:
    ...
    def trigger_manual_import(
        ...
        """
        params = {"folder": folder_path, "filterExistingFiles": True}
        if movie_id:
    ...
    def import_movie(
        ...
        Returns:
            Import result
        """
        files = self.trigger_manual_import(
            str(Path(file_path).parent), movie_id, quality_profile_id
        )
    ...
    def upload_and_import(
        ...
        Handles full workflow: search TMDB, add to library, copy file, trigger import.
        ...
        if not movie and auto_search:
            ...
            print("Adding to Radarr library...")
        ...
        if not movie:
            raise ValueError("Movie not found in library and auto_search is disabled")
        ...
        result = self.import_movie(
            str(dest_file), movie["id"], quality_profile_id, copy_files=False
        )

        print("✓ Import completed successfully!")

        return {"movie": movie, "import_result": result, "file_path": str(dest_file)}
...
Examples:
  ...
  radarr-upload movie.mp4
    ...
    parser.add_argument("file", metavar="FILE", help="path to movie file to upload")
    ...
    parser.add_argument(
        "--move", action="store_true", help="move file instead of copying"
    )
    ...
    parser.add_argument(
        "--list-folders", action="store_true", help="list root folders and exit"
    )
...
def get_api_credentials(args: argparse.Namespace) -> tuple[str, str]:
    ...
    if not api_key:
        ...
        print(
            "Provide via --api-key or RADARR_API_KEY environment variable",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    ...
    try:
        ...
        print(f"\n✓ Success!")
        print(
            f"Movie: {result['movie']['title']} ({result['movie'].get('year', 'N/A')})"
        )
